 ...
 * 
 * Copyright (c) Varank Tech Ltd.
 * All rights reserved.
 * 
 * This software is the confidential and proprietary information of Varank Tech Ltd.
 * ("Confidential Information").  You shall not disclose such Confidential Information
 * and shall use it only in accordance with the terms of the license agreement
 * you entered into with Varank.
 */
...
public class Home{
	...
	/* Complex containers for storing logsheet */
	...
	/*
	 * Returns a verified Employee ID.
	 * Takes in a DBConnection object & inputs a employee ID, validates it,
	 * runs a select query to check for that Employee ID in 'm_employee_ap'.
	 * 
	 * @param 	connector	instance of the DBConnection used for establishing database Connection
	 * @return	empID		String Employee ID used as a search key for other employee properties
	 */
	...
	/*
	 * Accesses the following tables: (emplpoyee_desig_ap, employee_department_ap, employee_office_ap,
	 * 				employee_gender_ap, employee_acctype_ap, employee_marital_ap, employee_qualname_ap) 
	 * filling Hashset & Hashmap containers with data from above meta tables.
	 * 
	 * @param 	connector	instance of the DBConnection used for establishing database Connection
	 */
	...
	/*
	 * Provides a menu of Basic properties of employee to choose & edit from
	 * For each property/choice the EMployee ID is decided depending on the privileges
	 * available to the current user. This employeeID is passed to the prepared Statement
	 * along with name of the chosen field to update table - 'm_employee_ap'
	 * 
	 * @param 	beanObj		instance of the EmployeeBean used to store all Employee properties
	 * @param 	connector	instance of the DBConnection used for establishing database Connection
	 */
	...
	/*
	 * Provides a menu of Bank properties of employee to choose & edit from
	 * For each property/choice the Employee ID is decided depending on the privileges
	 * available to the current user. This employeeID is passed to the prepared Statement
	 * along with name of the chosen field to update table - 'm_employee_bank_ap'
	 * Very similar to the editBasic() function.
	 * 
	 * @param 	beanObj		instance of the EmployeeBean used to store all Employee properties
	 * @param 	connector	instance of the DBConnection used for establishing database Connection
	 */	
	...
	public static void addEducation(EmployeeBean beanObj, DBConnection connector) {
		...
		try {
			...
			while(true) {
				...
				//				System.out.println("Checking whether qualification already exists . . . ");
	...
	/*
	 * Provides a menu to view different properties of an individual employee
	 * First it decides on the Employee ID based on the privileges available to the current user.
	 * In accordance with the choice made, this employeeID is utitlised to fetch data from among
	 * 3 tables - Basic Details: 'm_employee_ap', Bank Details: 'm_employee_bank_ap', Qualification Details: 'm_employee_qual_ap'
	 * 
	 * For each Employee ID Basic & Bank details shall be present, but since qualifications are added afterwards
	 * a query to 'm_employee_qual_ap' may return an empty Result Set.
	 * 
	 * @param 	beanObj		instance of the EmployeeBean used to store all Employee properties
	 * @param 	connector	instance of the DBConnection used for establishing database Connection
	 */
	...
	/*
	 * Provides a full record of logIN & logOUT entries of users using the application along with the duration
	 * Users with admin privileges can view all records, a general user can view only his/her records.
	 * First it decides on the Employee ID based on the 'query' to run depending on privilege available to the current user.
	 * The query is made to 'm_employee_users_ap'
	 * 
	 * Duration is initially achieved in milliseconds then converted to 'X mins Y secs'
	 * 
	 * @param 	beanObj		instance of the EmployeeBean used to store all Employee properties
	 * @param 	connector	instance of the DBConnection used for establishing database Connection
	 */
	public static void viewLogSheet(DBConnection connector, EmployeeBean beanObj) {
		...
		// Stores (username: time in milliseconds)
		...
		try {
			try {
				...
				while(rs.next()) {
					...
					if(totalTimeMap.get(rs.getString(1))==null) //if key for this username does not exist
						totalTimeMap.put(rs.getString(1), diff);
					else
						totalTimeMap.put(rs.getString(1), totalTimeMap.get(rs.getString(1))+diff); //if key for this username exists already

					long diffMins = diff/60000;
					long diffSecs = (diff - (diffMins * 60000))/1000;

					System.out.format("%4s %12s %15s %12s %12s %14s\n", counter++ +")",
							rs.getString(1), rs.getDate(2), rs.getTime(3), rs.getTime(4), diffMins +"m "+diffSecs+ "s");
	...
	/*
	 * Prepares the application for a system exit call.
	 * Creates an entry in the logsheet table for the logIN & exit/logOUT time of the current user along with current date.
	 * 
	 * Complex structure of nested HashMap is used here for demonstration purpose only,
	 * A simpler measure could have been written to perform the same task.
	 * 
	 * @param	timeBeanObj		instance of EmployeeTimeBean used to store timeIN, timeOUT of the current user
	 * @param	timeArr			ArrayList to store timeBeanObjects (only single Obj being stored here)
	 * @param 	beanObj			instance of EmployeeBean used to store currentUser & currentDate alongside other Emp properties
	 * @param 	connector		instance of DBConnection used for establishing database Connection
	 */
	 ...
	 * Creates a record for basic details, bank details & user credentials with privileges in
	 * 'm_employee_ap', 'm_employee_bank_ap', 'm_employee_users_ap' respectively.
	 * 
	 * 
	 * @param	newUserEmpID		a verified Employee ID string passed to the function after checking database for duplicates
	 * 
	 * (loaded from meta tables in fillpresets())
	 * @param	desigMap			map containing list of possible designations as (designation: rank/hierarchy)
	 * @param 	maritalStatusMap	map containing list of possible marital-status as (yes/no : 1/0)
	 * @param 	deptSet				set containing list of possible departments
	 * @param 	genderSet			set containing list of possible genders
	 * @param 	officeLocSet		set containing list of possible office locations
	 * @param 	accTypeSet			set containing list of possible account types
	 */
	public static void addEmployee(String newUserEmpID, HashMap<String,Integer> desigMap, HashMap<String,Integer> maritalStatusMap,
			HashSet<String> deptSet, HashSet<String> genderSet, HashSet<String> officeLocSet, HashSet<String> accTypeSet) throws InvalidInputException, EmptyInputException, SQLException{
	...
	/*
	 * The program starts here. All menu(s) nest inside main.
	 * All above functions are called inside main.
	 */
	public static void main(String[] args){

		boolean loggerExit=true; // used to escape while loop after successful login
